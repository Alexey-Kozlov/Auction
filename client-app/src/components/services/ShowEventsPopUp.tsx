...
import { useElkIndexMutation, useSetSnapShotMutation, useRestoreSnapShotMutation,
  useResetImageCacheMutation } from '../../api/ServiceApi';
...
export default function ShowEventsPopUp() {
  ...
  useEffect(() => {
    ...
    if(events.find(p => p.eventName === 'RestoreSnapShot' && !p.ready)){
      let restoreData = events.find(p => p.eventName === 'RestoreSnapShot' && !p.ready)?.param;
      const data: RestoreDb = {sessionid: sessionId, resetLog: restoreData.resetLog, 
