...
using MassTransit;
using ProcessingService.Activities.TagCreate;
...
public class CreateTagStateMachine : MassTransitStateMachine<CreateTagState>
{
    ...
    public CreateTagStateMachine(IServiceProvider services)
    {
    ...
    {
        //поступил запрос на создание тега для аукциона
        Initially(
    ...
    private void ConfigurePreCommitState()
    {
        ...
        When(CommitEvent)
            .Publish(context => new TagCreateESCommit
            {
                CorrelationId = context.Saga.CorrelationId
            })
            ...
            r => r
                ...
                //Запрашиваем обновленный список тегов
                    .Publish(context => new TagListRequest
                    {
                        ...
                        CorrelationId = context.Saga.CorrelationId,
                        CallBackType = "Common.Contracts.Tag.TagCreateNotificationEvent",
                    })
