...
public class TagDeleteProcessing
{
    ...
    public TagDeleteProcessing(IPublishEndpoint publishEndpoint, EventSourcingDbContext dbContext,
    ...
    public async Task ProcessESLog(ConsumeContext<ESContract> context)
    {
        ...
        try
        {
            var result = await _dbContext.tag_delete(
        ...
        catch (Exception e)
        {
            ...
            messageObject.GetType().GetProperty("ErrorServiceName").SetValue(messageObject, "EventSourcingService_DeleteTag");
