using System.Reflection;
using Common.Contracts.Image;
using Common.Contracts.Processing;
using ImageService.Data;
using MassTransit;
using Microsoft.EntityFrameworkCore;

namespace ImageService.Consumers;

public class CommitImageConsumer : IConsumer<ImageCommit>
{
    private readonly ImageDbContext _dbContext;
    private readonly IPublishEndpoint _publishEndpoint;
    private readonly IConfiguration _configuration;

    public CommitImageConsumer(ImageDbContext dbContext, IPublishEndpoint publishEndpoint, IConfiguration configuration)
    {
        _dbContext = dbContext;
        _publishEndpoint = publishEndpoint;
        _configuration = configuration;
    }
    public async Task Consume(ConsumeContext<ImageCommit> context)
    {
        _dbContext.ChangeTracker.Clear();
        var correlationId = context.Message.CorrelationId;
        try
        {
            foreach (var item in await _dbContext.Images.Where(p =>
                p.CorrelationId == context.Message.CorrelationId).ToListAsync())
            {
                if (context.Message.Commited)
                {
                    //подтверждение транзакции
                    if (item.Commited)
                    {
                        //удаляем старые записи
                        _dbContext.Images.Remove(item);
                    }
                    else
                    {
                        //подтверждаем новые записи
                        item.Commited = true;
                    }
                }
                else
                {
                    //откат транзакции
                    if (!item.Commited)
                    {
                        //удаляем новые записи
                        _dbContext.Images.Remove(item);
                    }
                }
            }

            await _dbContext.SaveChangesAsync();
        }
        catch (Exception e)
        {
            var errorItem = new NotificationServiceError
            {
                CorrelationId = context.Message.CorrelationId,
                Message = e.Message,
                ExceptionMessage = e.Source + "," + e.StackTrace,
                ServiceName = "ImageService",
                UserLogin = "",
                CallBackType = "",
                IsError = true
            };
            await _publishEndpoint.Publish(errorItem);
        }
    }
}
